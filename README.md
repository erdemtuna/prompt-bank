...
- To cut a release, bump `package.json`, which is also the Tauri application version source, and push a matching version tag, for example `git tag v0.5.1 && git push origin v0.5.1`. Keep the desktop crate version and lockfile entry in sync. The workflow creates one release, has every platform job upload its installer to that same release, and then publishes it as the latest release only after all builds succeed. macOS is built for both Apple Silicon and Intel.
